...
inline HttpHeadersFrame::HttpHeadersFrame(const std::shared_ptr<HttpHPacker> &packer)
    : HttpFrame(HttpFrame::Type::Header), d_padLength(0), d_isDepMutuallyExclusive(false), d_streamDep(0), d_weight(0),
      d_packer(packer)
{
    d_length += 6;
}

inline void HttpHeadersFrame::addFlag(Flag flag) { d_flags |= flag; }

inline void HttpHeadersFrame::clearHeaderBlocks() { d_headerList.clear(); }

inline std::vector<HttpHeaderField_t>::const_iterator HttpHeadersFrame::beginHeaderList() const
{
    return d_headerList.cbegin();
}

inline std::vector<HttpHeaderField_t>::const_iterator HttpHeadersFrame::endHeaderList() const
{
